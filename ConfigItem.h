...
public:
	...
	ConfigItem(const char *sName, int nType, void *pValue, bool bInitUpdate = false);

	// integer array (resolution, flowmap size, textures)
	// float array (effect prefs)
	static ConfigItem *Int(const char *sName, int *pnValue, bool bInitUpdate = false);
	static ConfigItem *Bool(const char *sName, bool *pbValue, bool bInitUpdate = false);
	static ConfigItem *Float(const char *sName, float *pfValue, bool bInitUpdate = false);
	static ConfigItem *String(const char *sName, std::string *psValue, bool bInitUpdate = false);
	~ConfigItem();
