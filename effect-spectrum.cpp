...
public:
	FLOAT32 m_fBrAng;
	ZCamera m_cCamera;
	ZObject m_pObj[TRAIL_H];
	ZObject m_pLimit[LIMITER_H];
	FLOAT32 m_fAng;
	FLOAT32 m_fRotAng;
	...
	* Constructor:
	...
	ZEffectSpectrum( )
	{
		...
		for(int i = 0; i < TRAIL_H; i++)
		{
			...
			m_pObj[i].m_bsFlag.set(ZObject::F_DRAW_TRANSPARENT);
			m_pObj[i].m_bsFlag.set( ZObject::F_DRAW_Z_BUFFER, false );
			WORD v = 0, f = 0;
			for(int j = 0; j < TRAIL_W; j++)
			{
				int n = j * 4;
				static const FLOAT32 s = 0.5f / sqrtf( 2.0f );
				m_pObj[ i ].pVertex[ n + 0 ].m_vNormal = ZVector( 0.0f, -s, -s );
				m_pObj[ i ].pVertex[ n + 1 ].m_vNormal = ZVector( 0.0f, +s, -s );
				m_pObj[ i ].pVertex[ n + 2 ].m_vNormal = ZVector( 0.0f, +s, +s );
				m_pObj[ i ].pVertex[ n + 3 ].m_vNormal = ZVector( 0.0f, -s, +s );
				...
				if(j < TRAIL_W - 1)
				{
					...
					for( WORD k = 0; k < 4; k++ )
					{
						...
						( *pFace )[ 0 ] = ( WORD )( v + k );
						( *pFace )[ 1 ] = ( WORD )( v + k + 4 );
						( *pFace )[ 2 ] = ( WORD )( v + ( ( k + 1 ) % 4 ) );

						pFace = &m_pObj[ i ].pFace[ f + k * 2 + 1 ];
						( *pFace )[ 0 ] = ( WORD )( v + ( ( k + 1 ) % 4 ) );
						( *pFace )[ 1 ] = ( WORD )( v + k + 4 );
						( *pFace )[ 2 ] = ( WORD )( v + 4 + ( ( k + 1 ) % 4 ) );
					}
					f += 8;
				}
				v += 4;
		...
		for(int i = 0; i < LIMITER_H; i++)
		{
			...
			m_pLimit[i].m_bsFlag.set(ZObject::F_DRAW_TRANSPARENT);
			m_pLimit[i].m_bsFlag.set(ZObject::F_DRAW_Z_BUFFER, false );
			m_pLimit[i].m_bsFlag.set(ZObject::F_DRAW_VERTEX_SPRITES);
...
//			FLOAT32 dMult = ((sin((CUBE_H + i + (dAng / TRAIL_ANGS)) * 3.14159 / TRAIL_H) * 0.7) + 0.8) * TRAIL_XS;
	...
	* Calculate( ):
	...
	ZError* Calculate( FLOAT32 fBrightness, FLOAT32 fElapsed, ZAudio* pAudio) override
	{
		...
		ZVector vDir = ZVector::Origin() - m_cCamera.m_vPosition;
		...
		for(i = 0; i < TRAIL_H; i++)
		{
			...
			for(int j = 0; j < TRAIL_W; j++)
			{
				...
				m_pObj[ i ].pVertex[ n + 0 ].m_vPos = ZVector( fX, fY - BAR_SIZE, fZ - BAR_SIZE );
				m_pObj[ i ].pVertex[ n + 1 ].m_vPos = ZVector( fX, fY + BAR_SIZE, fZ - BAR_SIZE );
				m_pObj[ i ].pVertex[ n + 2 ].m_vPos = ZVector( fX, fY + BAR_SIZE, fZ + BAR_SIZE );
				m_pObj[ i ].pVertex[ n + 3 ].m_vPos = ZVector( fX, fY - BAR_SIZE, fZ + BAR_SIZE );
		...
		for( i = 0; i < LIMITER_H; i++ )
		{
			...
			m_pLimit[ i ].wcAmbientLight = ZColour::Grey( ( int )( 255.0f * fBr * fBrightness ) );
	...
	* Reconfigure( ):
	...
	ZError* Reconfigure(ZAudio* pAudio) override
	{
		...
		for(int i = 0; i < TRAIL_H; i++)
		{
			m_pObj[i].pTexture[0].Set(ZObject::Texture::T_ENVMAP, pTexture);
		}
		for(int i = 0; i < LIMITER_H; i++)
		{
