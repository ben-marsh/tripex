...
namespace XBoxDX8
{
	extern "C" 
	{
		__declspec( dllimport ) HRESULT WINAPI D3DXCreateTextureFromFileInMemory( LPDIRECT3DDEVICE9 pDevice, LPCVOID pSrcData, UINT SrcData, LPDIRECT3DTEXTURE9* ppTexture );
		__declspec( dllimport ) HRESULT WINAPI D3DXCreateTexture( LPDIRECT3DDEVICE9 pDevice, UINT Width, UINT Height, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, LPDIRECT3DTEXTURE9* ppTexture );
	};
};
...
HRESULT ZDirect3D::FlushTextureState()
{
		...
		for(DWORD dwStage = 0; dwStage < 8; dwStage++)
		{
			...
			// SetSamplerState
//TODO			g_pDevice->SetTextureStageState(dwStage, D3DTSS_MIPFILTER, D3DTEXF_POINT);
