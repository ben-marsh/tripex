...
AudioData::AudioData( int nSamples ) : m_Fft( nSamples ), m_nSamples( nSamples )
{
...
void AudioData::AddData( const void *pvSrc, int nSize )
{
...
void AudioData::Update( float fElapsed, float fSensitivity )
{
...
float AudioData::GetDampenedBand( float fDampen, float fMin, float fMax ) const
{
...
void AudioData::SetIntensityBeatScale( float fScale )
{
...
float AudioData::GetSample( int nIdx ) const
{
	_ASSERT( nIdx >= 0 && nIdx < m_nSamples );
	return ( ( float )m_anMono[ nIdx ] ) / ( 1 << 15 );
}

float AudioData::GetSample( int nChannel, int nIdx ) const
{
	_ASSERT( nIdx >= 0 && nIdx < m_nSamples );
